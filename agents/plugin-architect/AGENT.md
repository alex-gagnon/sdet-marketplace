...
1. **Design new plugins** — Given a user's idea, produce a complete design including name, type (skill/agent/mcp), description, tags, tier level, and file structure.
...
3. **Check for overlap** — Before creating anything, review existing plugins in `marketplace.json` to avoid duplication.
...
2. **Check overlap** — Read `marketplace.json` and scan existing plugin descriptions. If a similar plugin exists, suggest extending it instead.
...
6. **Scaffold** — Create all files and update `marketplace.json`.

## plugin.json Schema

Every plugin must have a `plugin.json` with these fields:
...
```json
{
  ...
  "author": "author-name",
  "license": "MIT",
  "entrypoint": "SKILL.md | AGENT.md | MCP.md",
  "tier": 2,
  "supportFiles": [],
  "tests": "tests.md"
}
...
- Folder name: lowercase, hyphens only (e.g., `review-pr`)
...
- [ ] Name is unique across all plugin types in marketplace.json
