...
- Detects and reuses existing pytest fixtures from `conftest.py` rather than overwriting them
- Supports `@pytest.mark.parametrize` for data-driven E2E tests and custom markers for test categorisation
...
- **Reuse existing pytest fixtures**: before generating `conftest.py`, read any existing one and use the fixture names and scopes already defined; never overwrite existing fixtures
...
- **Use `@pytest.mark.parametrize`** for data-driven E2E tests where the same flow is exercised with multiple input sets (e.g. login with multiple credential pairs); follow the parametrize patterns in the loaded template
- **Apply pytest markers** (`@pytest.mark.smoke`, `@pytest.mark.regression`, etc.) to tests when the project's `pytest.ini` or `pyproject.toml` declares custom markers; read those files before generating
